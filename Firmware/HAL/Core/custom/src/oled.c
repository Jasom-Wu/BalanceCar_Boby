...
{
	i2c_Start(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);
	i2c_SendByte(OLED_ADDRESS,OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);
	if(i2c_WaitAck(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN) !=0)
	goto fail;
	i2c_SendByte(0x00,OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);//对于OLCD,0x00是指令的基地址，写入的指令执行一次，下一次写入时就覆盖了
	if(i2c_WaitAck(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN) !=0)
	goto fail;
	i2c_SendByte(cmd,OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);
	if(i2c_WaitAck(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN) !=0)
	goto fail;
	i2c_Stop(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);
	return 1;
	fail:
	i2c_Stop(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);
	return 0;
...
int OLED_SEND_Data(uint8_t data)//写操作不同设备不一样,所以要自己定义
{
	...
	if(i2c_WaitAck(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN) !=0)
	goto fail;
	i2c_SendByte(0x40,OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);//对于OLCD,0x40是数据域的基地址
	if(i2c_WaitAck(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN) !=0)
	goto fail;
	i2c_SendByte(data,OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);
	if(i2c_WaitAck(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN) !=0)
	goto fail;
	i2c_Stop(OLED_GPIO_PORT, OLED_GPIO_SDA_PIN,OLED_GPIO_CLK_PIN);
	return 1;
...
{
	I2C_GPIO_Config(OLED_GPIO_PORT , OLED_GPIO_SDA_PIN , OLED_GPIO_CLK_PIN);
