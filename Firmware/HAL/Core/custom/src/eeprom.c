...
{
	uint8_t c[sizeof(float)] = {0},flag;
	uint16_t x = *index;
	for(int x=0;x<sizeof(float);x++)
	c[x] = *((uint8_t *)(&f)+x);
	flag = EEPROM_WriteBytes(c ,x, sizeof(float));
	*index += sizeof(float);
	return flag;
}
...
void EEPROMReload(void)
{
	...
	index+=5;
	EEPROM_ReadBytes((uint8_t *)&User_Data.Baudrate ,index, 1);
	index+=1;
	...
	Mechanical_balance = EEPROMReadFloat((uint16_t *)&index);
}
void EEPROMOutload(void)
{
	...
	EEPROMWriteFloat(Mechanical_balance,(uint16_t *)&index);
}
void EEPROM_Config(void)
{
	I2C_GPIO_Config(EEPROM_GPIO_PORT ,EEPROM_GPIO_SDA_PIN,EEPROM_GPIO_SCL_PIN);
}
